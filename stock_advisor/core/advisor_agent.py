...
try:
    ...
    from stock_advisor.core.demo_data import generate_demo_stock_data, get_demo_stock_info, get_demo_news_sentiment, get_demo_3day_predictions
except ImportError:
    # Fallback to relative imports if running from within the package
    from .stock_data import StockDataFetcher
    from .news_collector import NewsCollector
    from .predictor import StockPredictor
    from .demo_data import generate_demo_stock_data, get_demo_stock_info, get_demo_news_sentiment, get_demo_3day_predictions
...
class StockAdvisorAgent:
    ...
    def analyze_stock(self, symbol: str, period: str = "3mo") -> Dict:
        ...
        try:
            # Get stock data (use demo data if in demo mode or if real data fails)
            ...
            stock_data_with_indicators = self.stock_fetcher.calculate_technical_indicators(stock_data)
            ...
            training_results = self.predictor.train_models(stock_data_with_indicators, stock_sentiment)
            
            # Get predictions for next 3 days
            if self.demo_mode:
                print("(Using demo predictions)")
                current_price = stock_data_with_indicators['Close'].iloc[-1]
                prediction_result = get_demo_3day_predictions(symbol, current_price)
            else:
                prediction_result = self.predictor.get_ensemble_prediction(stock_data_with_indicators, stock_sentiment, days=3)
            
            # Current market metrics
            current_data = stock_data_with_indicators.iloc[-1]
            previous_data = stock_data_with_indicators.iloc[-2] if len(stock_data_with_indicators) > 1 else current_data
            ...
            analysis.update({
                ...
                'current_metrics': {
                    ...
                    'price_change_pct': float((current_data['Close'] - previous_data['Close']) / previous_data['Close'] * 100),
                    'volume': int(current_data['Volume']),
                    'rsi': float(current_data.get('RSI', 0)) if not pd.isna(current_data.get('RSI', float('nan'))) else None,
                    'macd': float(current_data.get('MACD', 0)) if not pd.isna(current_data.get('MACD', float('nan'))) else None
                },
                'technical_analysis': self._analyze_technical_indicators(stock_data_with_indicators),
    ...
    def _generate_recommendation(self, analysis: Dict) -> Dict:
        ...
        if 'ensemble_prediction' in prediction:
            ...
            if overall_summary.get('overall_direction') == 'UP' and overall_summary.get('average_confidence', 0) > 60:
                score += 2
                recommendation['reasoning'].append(f"ML models predict {overall_summary['average_confidence']:.1f}% chance of price increase")
            elif overall_summary.get('overall_direction') == 'DOWN' and overall_summary.get('average_confidence', 0) > 60:
                score -= 2
                recommendation['reasoning'].append(f"ML models predict {overall_summary['average_confidence']:.1f}% chance of price decrease")
            max_score += 2
    ...
    def _generate_watchlist_summary(self, analyses: Dict) -> Dict:
        ...
        for symbol, analysis in analyses.items():
            ...
            stock_summary = {
                ...
                'current_price': analysis.get('current_metrics', {}).get('current_price', 0),
                'price_change_pct': analysis.get('current_metrics', {}).get('price_change_pct', 0)
            }
    ...
    def get_analysis_history(self, symbol: Optional[str] = None, days: int = 7) -> List[Dict]:
