...
class StockDataFetcher:
    def __init__(self):
        ...
        self.use_alpha_vantage = bool(self.alpha_vantage_key)
        self.iex_cloud_key = os.getenv('IEX_CLOUD_API_KEY')
        self.use_iex_cloud = bool(self.iex_cloud_key)
    ...
    def get_stock_data(self, symbol: str, period: str = "1mo") -> pd.DataFrame:
        ...
        for attempt in range(max_retries):
            try:
                ...
                data = self._fetch_yahoo_data_with_alternative_periods(ticker, period, symbol)
                if not data.empty:
                    self.cache[cache_key] = data
                    return data
    ...
    def get_stock_data_alpha_vantage(self, symbol: str, period: str = "1mo") -> pd.DataFrame:
        """
        ...
        try:
            ...
            if not time_series:
                print(f"No Alpha Vantage data for {symbol}: {data.get('Note', 'Unknown error')}")
                return pd.DataFrame()
    ...
    def get_stock_data_iex_cloud(self, symbol: str, period: str = "1mo") -> pd.DataFrame:
        """
        ...
        try:
            ...
            print(f"Successfully fetched {len(df)} days of data for {symbol} from IEX Cloud")
            return df
    ...
    def _fetch_yahoo_data_with_period(self, ticker, period: str, symbol: str) -> pd.DataFrame:
        """Fetch Yahoo Finance data with original period"""
        try:
            data = ticker.history(period=period, timeout=30)
            if not data.empty:
                print(f"✅ Yahoo Finance: {len(data)} days for {symbol} (period: {period})")
                return data
        except Exception as e:
            print(f"Period fetch failed: {e}")
        return pd.DataFrame()
    
    def _fetch_yahoo_data_with_alternative_periods(self, ticker, period: str, symbol: str) -> pd.DataFrame:
        ...
        for alt_period in alternatives[1:]:  # Skip first (already tried)
            try:
                ...
                if not data.empty:
                    print(f"✅ Yahoo Finance: {len(data)} days for {symbol} (alt period: {alt_period})")
                    return data
            except Exception as e:
    ...
    def _fetch_yahoo_data_with_dates(self, ticker, period: str, symbol: str) -> pd.DataFrame:
    ...
    def _try_alternative_yahoo_methods(self, symbol: str, period: str, cache_key: str) -> pd.DataFrame:
        ...
        try:
            ...
            for minimal_period in ["5d", "1mo", "3mo"]:
                try:
                    data = ticker.history(period=minimal_period, timeout=45, 
        ...
        try:
            ...
            if not short_data.empty:
                ...
                try:
                    ...
                    extended_data = ticker.history(period="2mo", interval="1d", timeout=45)
        ...
        try:
            ...
            if info and info.get('regularMarketPrice'):
                print(f"Ticker exists (price: ${info.get('regularMarketPrice')}), trying basic fetch...")
                
                # Try the most basic fetch possible
                data = ticker.history(period="1mo", auto_adjust=True, 
                                    back_adjust=False, timeout=60)
                if not data.empty:
    ...
    def _filter_data_to_period(self, data: pd.DataFrame, target_period: str) -> pd.DataFrame:
