...
class StockPredictor:
    ...
    def prepare_features(
        self, data: pd.DataFrame, news_sentiment: Optional[Dict] = None
    ) -> pd.DataFrame:
        """
    ...
    def train_models(
        ...
        if len(df_clean) < 20:
            return {
                "error": "Insufficient data for training (need at least 20 samples)"
            }
            ...
            for day in range(days):
                ...
                if day == 0:
                    ...
                    if "price_change" in synthetic_data.columns:
                        synthetic_data["price_change"] = predictions[day - 1][
                            "predicted_change"
                        ]
                ...
                confidence = (
                    abs(daily_prediction) * 100 * (1 - 0.1 * day)
    ...
    def get_ensemble_prediction(
        ...
        for model_name in self.trained_models.keys():
            result = self.predict_next_price(
                current_data, news_sentiment, model_name, days
            )
        ...
        for day in range(days):
            ...
            for model_name, prediction in individual_predictions.items():
                if "daily_predictions" in prediction and day < len(
        ...
        avg_uncertainty = np.mean(
            [p["uncertainty"] for p in ensemble_daily_predictions]
        )
