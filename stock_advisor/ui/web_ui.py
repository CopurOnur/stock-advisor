...
class StockAdvisorUI:
    ...
    def create_price_chart(
        self, stock_data: pd.DataFrame, predictions: dict, symbol: str
    ) -> go.Figure:
        ...
        if "daily_predictions" in predictions:
            pred_prices = [current_price] + [
                p["predicted_price"] for p in predictions["daily_predictions"]
            ]
            ...
            if method_key in results:
                ...
                if "daily_predictions" in method_data:
                    changes = [
                        ...
                        for p in method_data["daily_predictions"]
                    ]
    ...
    def create_backtest_performance_chart(self, backtest_results: Dict) -> go.Figure:
        ...
        fig.update_layout(
            title="7-Day Backtest Performance Comparison", height=400, showlegend=False
        )
    ...
    def display_daily_backtest_details(self, backtest_results: Dict):
        ...
        for method_key, method_name in method_names.items():
            if method_key in backtest_results["methods"]:
                with tabs[tab_idx]:
                    daily_results = backtest_results["methods"][method_key][
                        "daily_results"
                    ]
                    valid_results = [r for r in daily_results if "error" not in r]
                    ...
                    if valid_results:
                        ...
                        perf = backtest_results["methods"][method_key]["performance"]
                        st.write(
                            f"**Summary**: {perf['direction_correct_count']}/7 correct directions "
    ...
    def run_dashboard(self):
        ...
        if demo_mode:
            st.sidebar.info(
                "Demo mode uses simulated data - perfect for testing without API limits!"
            )
                ...
                try:
                    ...
                    if demo_mode:
                        ...
                        if not stock_data.empty:
                            stock_data = (
                                self.stock_fetcher.calculate_technical_indicators(
                                    stock_data
                                )
                            )
                    ...
                    if "Technical Analysis" in prediction_methods:
                        ...
                        if demo_mode:
                            ...
                            original_get_stock_data = (
                                self.technical_predictor.stock_fetcher.get_stock_data
                            )
                        ...
                        tech_result = self.technical_predictor.predict_next_3_days(
                            symbol
                        )
                    ...
                    if "Reinforcement Learning" in prediction_methods:
                        ...
                        if demo_mode:
                            # Override stock fetcher for demo mode
                            original_get_stock_data = (
                                self.rl_predictor.stock_fetcher.get_stock_data
                            )
                        ...
                        rl_result = self.rl_predictor.predict_next_3_days(
                            symbol, auto_train
                        )
                    ...
                    if "Enhanced RL" in prediction_methods:
                        ...
                        if demo_mode:
                            # Override stock fetcher for demo mode
                            original_get_stock_data = (
                                self.enhanced_rl_predictor.stock_fetcher.get_stock_data
                            )
                        ...
                        enhanced_rl_result = (
                            self.enhanced_rl_predictor.predict_next_3_days(
                                symbol, auto_train
                            )
                        )
                    ...
                    if "Hybrid" in prediction_methods:
                        ...
                        if demo_mode:
                            # Override stock fetcher for demo mode
                            original_get_stock_data = (
                                self.hybrid_predictor.stock_fetcher.get_stock_data
                            )
                    ...
                    if run_backtest:
                        ...
                        if demo_mode:
                            # Override stock fetcher for demo mode
                            original_get_stock_data = (
                                self.backtest_simulator.stock_fetcher.get_stock_data
                            )
                        ...
                        backtest_results = self.backtest_simulator.run_7_day_backtest(
                            symbol
                        )
        ...
        if "results" in st.session_state and st.session_state["results"]:
            ...
            # Show data source
            data_source = (
                "Demo Data" if demo_mode else "Live Data (Yahoo Finance/IEX Cloud)"
            )
            ...
            with col1:
                st.metric(
                    "Current Price", f"${current_price:.2f}", f"{daily_change:+.2f}%"
                )
            with col2:
            ...
            if "hybrid" in results and results["hybrid"].get("news_analysis_included"):
                ...
                if news_data:
                    ...
                    with col2:
                        st.metric(
                            "Articles Analyzed", news_data.get("news_articles_count", 0)
                        )
                        st.metric(
                            "Positive Keywords", news_data.get("positive_keywords", 0)
                        )
                        st.metric(
                            "Negative Keywords", news_data.get("negative_keywords", 0)
                        )
                        st.metric(
                            "Sentiment Confidence",
                            f"{news_data.get('confidence', 0)*100:.1f}%",
                        )
            ...
            tabs = st.tabs(
                [method.title().replace("_", " ") for method in results.keys()]
            )

            for i, (method_key, method_name) in enumerate(
                [(k, k.replace("_", " ").title()) for k in results.keys()]
            ):
                with tabs[i]:
                    ...
                    if (
                        method_key == "hybrid"
                        and "daily_predictions" in results[method_key]
                    ):
                            ...
                            with st.expander(f"Day {pred['day']} Details"):
                                if "method_predictions" in pred:
                                    for method_pred in pred["method_predictions"]:
                                        st.write(
                                            f"**{method_pred['method']}**: {method_pred['direction']} "
                                            f"(Confidence: {method_pred['confidence']:.1f}%)"
                                        )

                                if (
                                    "technical_signals" in pred
                                    and pred["technical_signals"]
                                ):
                        ...
                        with col1:
                            st.metric(
                                "Models Loaded",
                                f"{model_info.get('models_loaded', 0)}/{model_info.get('ensemble_size', 0)}",
                            )
                        with col2:
                            st.metric(
                                "PyTorch Used",
                                (
                                    "Yes"
                                    if model_info.get("pytorch_used", False)
                                    else "No"
                                ),
                            )
                        with col3:
                            st.metric(
                                "Consensus Strength",
                                f"{results[method_key]['overall_summary'].get('consensus_strength', 0):.1%}",
                            )
                        ...
                        for pred in results[method_key]["daily_predictions"]:
                            with st.expander(f"Day {pred['day']} Ensemble Details"):
                                ...
                                st.write(
                                    f"**Ensemble Size**: {pred['ensemble_size']} agents"
                                )
                                st.write(
                                    f"**Consensus Strength**: {pred['consensus_strength']:.1%}"
                                )
            ...
            if (
                "backtest_results" in st.session_state
                and st.session_state["backtest_results"]
            ):
                ...
                if "error" not in backtest_results:
                    ...
                    with col1:
                        performance_chart = self.create_backtest_performance_chart(
                            backtest_results
                        )
                        st.plotly_chart(performance_chart, use_container_width=True)

                    with col2:
                        daily_error_chart = self.create_daily_backtest_chart(
                            backtest_results
                        )
                    ...
                    for method_key in ["technical", "rl", "hybrid"]:
                        if method_key in backtest_results["methods"]:
                            count = len(
                                backtest_results["methods"][method_key]["daily_results"]
                            )
                    ...
                    if total_predictions:
                        ...
                        if max_predictions < 7:
                            st.warning(
                                f"⚠️ Only {max_predictions}/7 days of predictions available. This may be due to insufficient historical data."
                            )
                        else:
                            st.success(
                                f"✅ All 7 days of predictions completed for testing."
                            )
                    ...
                    for method_key in ["technical", "rl", "hybrid"]:
                        if method_key in backtest_results["methods"]:
                            perf = backtest_results["methods"][method_key][
                                "performance"
                            ]
                            if (
                                "error" not in perf
                                and perf["direction_accuracy"] > best_accuracy
                            ):
                    ...
                    if best_method:
                        st.success(
                            f"🏆 **Best Performer**: {best_method} with {best_accuracy:.1f}% direction accuracy"
                        )
            ...
            # Risk disclaimer
            st.warning(
                "⚠️ **Risk Disclaimer**: These predictions are for educational purposes only and should not be considered financial advice. Always do your own research and consult with qualified financial advisors before making investment decisions."
            )
