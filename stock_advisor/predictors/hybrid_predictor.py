...
class TechnicalAnalysisModule:
    ...
    def analyze_technical_signals(self, data: pd.DataFrame) -> Dict:
        ...
        if "SMA_5" in data.columns:
            if latest["Close"] > latest["SMA_5"] > latest["SMA_10"] > latest["SMA_20"]:
                ...
                latest["Close"] < latest["SMA_5"] < latest["SMA_10"] < latest["SMA_20"]
            ):
                signals["direction_votes"].append(-1)
                signals["confidence_scores"].append(0.8)
    ...
    def predict_technical_direction(
        ...
        weighted_sum = sum(
            vote * conf
            for vote, conf in zip(
                signals["direction_votes"], signals["confidence_scores"]
            )
        )
...
class HybridDQNAgent:
    ...
    def __init__(
        self, state_size: int = 15, action_size: int = 3, learning_rate: float = 0.001
    ):
...
class HybridStockPredictor:
    ...
    def predict_next_3_days(
        self, symbol: str, auto_train: bool = True, use_news: bool = True
    ) -> Dict:
        """
    ...
    def _predict_single_day(
        ...
        tech_direction, tech_confidence, tech_signals = (
            self.technical_module.predict_technical_direction(stock_data)
        )

        # 2. News Sentiment Prediction
        news_direction, news_confidence, news_explanation = (
            self.news_module.predict_news_direction(news_analysis)
        )
            ...
            prediction_details.append(
                {
                    "method": method,
                    "direction": (
                        "UP" if direction > 0 else "DOWN" if direction < 0 else "FLAT"
                    ),
        ...
        return {
            ...
            "news_explanation": (
                news_explanation if news_analysis else "News analysis disabled"
            ),
    ...
    def print_prediction_summary(self, result: Dict):
        ...
        print(
            f"News Analysis: {'Included' if result['news_analysis_included'] else 'Disabled'}"
        )
        ...
        print(
            f"{'Day':<5} {'Direction':<10} {'Price':<12} {'Change':<10} {'Confidence':<12} {'Ensemble':<10}"
        )
        print(f"{'-'*75}")
        ...
        for pred in result["daily_predictions"]:
            ...
            print(
                f"  Technical: {pred['technical_signals'][:2] if pred['technical_signals'] else 'No signals'}"
            )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python hybrid_predictor.py <SYMBOL> [--no-news] [--train-only] [--episodes=N]"
        )
        print("Examples:")
        print(
            "  python hybrid_predictor.py AAPL                    # Full hybrid prediction"
        )
        print(
            "  python hybrid_predictor.py AAPL --no-news          # Without news analysis"
        )
        print(
            "  python hybrid_predictor.py AAPL --train-only       # Only train the model"
        )
        print(
            "  python hybrid_predictor.py AAPL --episodes=100     # Train with 100 episodes"
        )
        return
    ...
    print(
        f"Methods: RL + Technical Analysis + {'News Sentiment' if use_news else 'No News'}"
    )
    ...
    if train_only:
        ...
        result = predictor.predict_next_3_days(
            symbol, auto_train=True, use_news=use_news
        )
    ...
    print(
        "still experimental. Not financial advice. Use for educational purposes only."
    )
    print(f"{'='*80}")
