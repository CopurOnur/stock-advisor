...
class TechnicalAnalysisPredictor(BasePredictor):
    ...
    def analyze_moving_averages(self, data) -> Dict:
        ...
        if 'SMA_5' in data.columns:
            signals['price_vs_sma5'] = 'above' if latest['Close'] > latest['SMA_5'] else 'below'
            signals['price_vs_sma10'] = 'above' if latest['Close'] > latest['SMA_10'] else 'below'
            signals['price_vs_sma20'] = 'above' if latest['Close'] > latest['SMA_20'] else 'below'
            
            # Moving average alignment (trend strength)
            ma_alignment = 'bullish' if (latest['SMA_5'] > latest['SMA_10'] > latest['SMA_20']) else 'bearish'
    ...
    def analyze_rsi(self, data) -> Dict:
        ...
        rsi_trend = 'rising' if data['RSI'].iloc[-1] > data['RSI'].iloc[-2] else 'falling'
    ...
    def predict_next_3_days(self, symbol: str) -> Dict:
        ...
        for day in range(1, 4):
            prediction = self._predict_single_day(
                day, current_price, ma_analysis, rsi_analysis, 
                macd_analysis, sr_analysis, volume_analysis, momentum
            )
        ...
        # Overall summary
        total_change = (daily_predictions[-1]['predicted_price'] - stock_data['Close'].iloc[-1]) / stock_data['Close'].iloc[-1]
    ...
    def _predict_single_day(self, day: int, current_price: float, ma_analysis: Dict, 
                           rsi_analysis: Dict, macd_analysis: Dict, sr_analysis: Dict, 
                           volume_analysis: Dict, momentum: Dict) -> Dict:
        ...
        if not direction_votes:
            ...
            base_change = 0.005 + (abs(direction) / len(direction_votes)) * 0.015  # 0.5% to 2%
        ...
        final_direction = 'UP' if predicted_change > 0 else 'DOWN' if predicted_change < 0 else 'FLAT'
    ...
    def print_prediction_summary(self, result: Dict):
        ...
        print(f"Position: {sr['position']} (Support: ${sr['support_level']:.2f}, Resistance: ${sr['resistance_level']:.2f})")
        ...
        # Daily Breakdown
        print(f"\nDAILY BREAKDOWN:")
        print(f"{'Day':<5} {'Direction':<10} {'Price':<12} {'Change':<10} {'Confidence':<12} {'Key Signals'}")
        print(f"{'-'*80}")
