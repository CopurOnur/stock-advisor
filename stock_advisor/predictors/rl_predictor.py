...
class StockTradingEnvironment:
    ...
    def _get_state(self) -> np.ndarray:
        ...
        state = [
            ...
            max(self._get_portfolio_value() / self.initial_balance, 0)  # Portfolio ratio
        ]
...
class RLStockPredictor:
    ...
    def predict_next_3_days(self, symbol: str, auto_train: bool = True) -> Dict:
        ...
        if not model_loaded and auto_train:
            ...
            return {'error': f'No trained model for {symbol}. Use auto_train=True or train manually.'}
    ...
    def print_prediction_summary(self, result: Dict):
        ...
        print(f"\nRL MODEL INFO:")
        print(f"Model Status: {'Loaded' if model_info['model_loaded'] else 'Newly Trained'}")
        ...
        print(f"{'Day':<5} {'Action':<6} {'Direction':<10} {'Price':<12} {'Change':<10} {'Confidence':<12}")
        print(f"{'-'*70}")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Examples:")
        print("  python rl_predictor.py AAPL                 # Predict (auto-train if needed)")
