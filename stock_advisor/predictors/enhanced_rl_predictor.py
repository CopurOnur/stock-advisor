...
class EnhancedTradingEnvironment:
    ...
    def _get_state(self) -> np.ndarray:
        ...
        window_data = self.data.iloc[max(0, current_idx - self.lookback_window):current_idx + 1]
    ...
    def _get_regime_features(self) -> float:
        ...
        window_data = self.data.iloc[max(0, self.current_step - 20):self.current_step + 1]
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        reward = self._calculate_reward(action, current_price, next_price, 
                                      initial_portfolio, final_portfolio, trade_made)
        ...
        if self.current_step >= len(self.data) - 1:
            ...
            total_return = (final_portfolio - self.initial_balance) / self.initial_balance
    ...
    def _calculate_reward(self, action: int, current_price: float, next_price: float,
                         initial_portfolio: float, final_portfolio: float, trade_made: bool) -> float:
    ...
    def _sell(self, price: float) -> Tuple[float, bool]:
        ...
        if self.shares_held > 0:
            ...
            avg_cost = (self.initial_balance - self.balance + self.shares_held * price) / max(self.shares_held, 1)
            if price > avg_cost:
...
class EnhancedDQNAgent:
    ...
    def __init__(self, state_size: int = 30, action_size: int = 3, 
                 learning_rate: float = 0.0001, use_pytorch: bool = True):
    ...
    def replay(self):
        ...
        if TORCH_AVAILABLE and self.use_pytorch:
            self._pytorch_train_step(states, actions, rewards, next_states, dones, weights, indices)
        else:
            self._simple_train_step(states, actions, rewards, next_states, dones, weights, indices)
    ...
    def _pytorch_train_step(self, states, actions, rewards, next_states, dones, weights, indices):
        ...
        loss = (weights.unsqueeze(1) * F.mse_loss(current_q_values, target_q_values.unsqueeze(1), reduction='none')).mean()
    ...
    def _simple_train_step(self, states, actions, rewards, next_states, dones, weights, indices):
...
class EnhancedRLStockPredictor:
    ...
    def __init__(self, use_pytorch: bool = True):
        ...
        for i in range(self.num_agents):
            agent = EnhancedDQNAgent(state_size=30, action_size=3, 
    ...
    def train_ensemble(self, symbol: str, episodes: int = 200) -> Dict:
        ...
        for agent_idx, agent in enumerate(self.agents):
            ...
            # Create environment
            env = EnhancedTradingEnvironment(stock_data, 
                                           transaction_cost=0.001 * (1 + agent_idx * 0.5))
            ...
            # Save trained agent
            model_path = os.path.join(self.model_dir, f'{symbol}_enhanced_rl_agent_{agent_idx}.pkl')
    ...
    def predict_next_3_days(self, symbol: str, auto_train: bool = True) -> Dict:
        ...
        if models_loaded == 0 and auto_train:
            ...
            return {'error': f'No trained models for {symbol}. Use auto_train=True or train manually.'}
        ...
        for agent_idx, agent in enumerate(self.agents):
            if agent_idx < models_loaded:
                ...
                steps = 0
                while not env.done and env.current_step < len(env.data) - 10 and steps < 100:
                    action = agent.act(state)
                ...
                for day in range(1, 4):
                    ...
                    if action == 1:  # BUY
                        ...
                        base_change = np.random.normal(-0.01, 0.005)  # 1% down with noise
                        confidence = 75 + np.random.normal(0, 5)
                        direction = 'DOWN'
                    else:  # HOLD
                        base_change = np.random.normal(0, 0.003)  # Flat with small noise
        ...
        for day in range(1, 4):
            ...
            avg_price = sum(p['predicted_price'] * p['confidence'] for p in day_predictions) / total_weight
            avg_change = sum(p['predicted_change'] * p['confidence'] for p in day_predictions) / total_weight
            ...
            daily_predictions.append({
                ...
                'consensus_strength': direction_counts[consensus_direction] / len(day_predictions)
            })
        ...
        return {
            ...
            'overall_summary': {
                ...
                'consensus_strength': np.mean([p['consensus_strength'] for p in daily_predictions])
            }
        }
    ...
    def print_prediction_summary(self, result: Dict):
        ...
        print(f"Models Loaded: {model_info['models_loaded']}/{model_info['ensemble_size']}")
        ...
        print(f"{'Day':<5} {'Direction':<10} {'Price':<12} {'Change':<10} {'Confidence':<12} {'Consensus':<10}")
        print(f"{'-'*80}")
...
def main():
    """Main function"""
    if len(sys.argv) < 2:
        print("Usage: python enhanced_rl_predictor.py <SYMBOL> [--train-only] [--episodes=N]")
        print("Examples:")
        print("  python enhanced_rl_predictor.py AAPL                 # Predict (auto-train if needed)")
        print("  python enhanced_rl_predictor.py AAPL --train-only    # Only train the ensemble")
        print("  python enhanced_rl_predictor.py AAPL --episodes=200  # Train with 200 episodes")
        return
    ...
    if train_only:
        ...
        if 'error' not in result:
            ...
            for agent_result in result['ensemble_results']:
                print(f"  Agent {agent_result['agent_id']}: "
    ...
    print(f"\n{'='*80}")
    print("DISCLAIMER: Enhanced RL predictions are experimental and for educational use.")
    print("Not financial advice. Deep learning models require significant training data.")
    print(f"{'='*80}")
