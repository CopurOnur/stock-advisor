...
class BacktestSimulator:
    ...
    def simulate_single_day_prediction(
        self,
        symbol: str,
        data_until_date: pd.DataFrame,
        ...
        try:
            if method == "technical":
                ...
                self.hybrid_predictor.stock_fetcher.cache[f"{symbol}_3mo"] = (
                    data_until_date
                )

                result = self.hybrid_predictor.predict_next_3_days(
                    symbol, auto_train=False, use_news=False
                )
                if "error" in result or not result.get("daily_predictions"):
            ...
            actual_change_pct = (
                (actual_next_price - current_price) / current_price * 100
            )
            actual_direction = (
                "UP"
                if actual_change_pct > 0
                else "DOWN" if actual_change_pct < 0 else "FLAT"
            )
    ...
    def run_7_day_backtest(self, symbol: str) -> Dict:
        ...
        if len(stock_data) < 15:
            return {
                "error": "Insufficient data for backtesting (need at least 15 days)"
            }
        ...
        training_data = (
            stock_data.iloc[:-14] if len(stock_data) > 50 else stock_data.iloc[:-10]
        )
        ...
        for day in range(7):
            ...
            if prediction_day_idx >= len(test_data):
                print(
                    f"  Day {day+1}/7: Insufficient data (need index {prediction_day_idx}, have {len(test_data)})"
                )
                continue
            ...
            print(
                f"  Day {day+1}/7: Predicting {test_date} using data up to {prediction_date}"
            )
            print(
                f"    Context data: {len(data_until_date)} days, Actual price: ${actual_next_price:.2f}"
            )
        ...
        for method in ["technical", "rl", "hybrid"]:
            ...
            if valid_results:
                ...
                results["methods"][method]["performance"] = {
    ...
    def _calculate_performance_metrics(self, daily_results: List[Dict]) -> Dict:
        ...
        direction_correct = sum(
            1 for r in daily_results if r.get("direction_correct", False)
        )
        ...
        price_errors_pct = [
            r["price_error_pct"] for r in daily_results if "price_error_pct" in r
        ]

        # Confidence metrics
        confidences = [r["confidence"] for r in daily_results if "confidence" in r]
        weighted_accuracies = [
            r["weighted_accuracy"] for r in daily_results if "weighted_accuracy" in r
        ]
        ...
        predicted_changes = [
            ...
            if "predicted_change_pct" in r
        ]
        actual_changes = [
            r["actual_change_pct"] for r in daily_results if "actual_change_pct" in r
        ]
        ...
        return {
            ...
            "weighted_accuracy": (
                np.mean(weighted_accuracies) if weighted_accuracies else 0
            ),
            ...
            "prediction_correlation": (
                np.corrcoef(predicted_changes, actual_changes)[0, 1]
    ...
    def print_backtest_summary(self, results: Dict):
        ...
        print(f"\nPERFORMANCE SUMMARY:")
        print(
            f"{'Method':<20} {'Direction Acc':<15} {'Avg Error %':<12} {'Confidence':<12} {'Correlation':<12}"
        )
        print(f"{'-'*75}")
        ...
        for method_name, method_key in [
            ...
            if valid_results:
                ...
                print(
                    f"{'Day':<5} {'Date':<12} {'Predicted':<12} {'Actual':<12} {'Error %':<10} {'Direction':<10} {'Correct':<8}"
                )
                print(f"{'-'*75}")
