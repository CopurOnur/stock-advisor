...
def test_data_sources():
    ...
    print(f"• IEX Cloud: {'✅ Configured' if fetcher.use_iex_cloud else '❌ Not configured'}")
    print(f"• Alpha Vantage: {'✅ Configured' if fetcher.use_alpha_vantage else '❌ Not configured'}")
    ...
    if not data.empty:
        ...
        print(f"   Daily change: {((data['Close'].iloc[-1] - data['Close'].iloc[-2]) / data['Close'].iloc[-2] * 100):+.2f}%")
