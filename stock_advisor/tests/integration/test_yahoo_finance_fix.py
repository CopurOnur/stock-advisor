    ...
    for symbol in test_symbols:
        ...
        try:
            ...
            if not data.empty:
                ...
                print(f"✅ {symbol}: {len(data)} days, ${data['Close'].iloc[-1]:.2f}, {elapsed:.2f}s")
            else:
            ...
            elapsed = time.time() - start_time
            results[symbol] = {'status': 'error', 'error': str(e), 'fetch_time': elapsed}
...
def test_different_periods():
    ...
    for period in periods:
        ...
        try:
            ...
            elapsed = time.time() - start_time
            results[period] = {'status': 'error', 'error': str(e), 'fetch_time': elapsed}
    ...
    for symbol in invalid_symbols:
        ...
        try:
            ...
            if data.empty:
                ...
                print(f"⚠️  {symbol}: Unexpectedly got data ({len(data)} days), {elapsed:.2f}s")
...
def test_technical_indicators():
    ...
    try:
        ...
        indicators = ['SMA_5', 'SMA_10', 'SMA_20', 'EMA_12', 'EMA_26', 'RSI', 'MACD', 'MACD_Signal']
        found_indicators = []
...
def test_caching():
    ...
    if second_fetch_time < first_fetch_time * 0.1:
        print(f"✅ Caching working properly (speedup: {first_fetch_time/second_fetch_time:.1f}x)")
        return True
    else:
        print(f"⚠️  Caching may not be working (speedup: {first_fetch_time/second_fetch_time:.1f}x)")
        return False
...
def run_comprehensive_test():
    ...
    if 'basic' in test_results:
        ...
        if successful > 0:
            avg_time = sum(r['fetch_time'] for r in basic_results.values() if r['status'] == 'success') / successful
