    ...
    for method in methods:
        if method in results["methods"]:
            ...
            if valid_results:
                ...
                print(
                    f"{'Day':<5} {'Test Date':<12} {'Pred Date':<12} {'Predicted':<12} {'Actual':<12} {'Correct':<8}"
                )
                print("-" * 70)
                ...
                for result in sorted(valid_results, key=lambda x: x["day"]):
                    ...
                    print(
                        f"{day:<5} {test_date:<12} {pred_date:<12} "
...
def test_different_symbols():
    ...
    for symbol in symbols:
        ...
        try:
            ...
            success_rate = (
                (total_valid / total_possible * 100) if total_possible > 0 else 0
            )
            print(
                f"  ✅ Success: {total_valid}/{total_possible} predictions ({success_rate:.1f}%)"
            )
