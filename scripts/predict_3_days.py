...
class ThreeDayPredictor:
    ...
    def predict_stock_3_days(self, symbol: str, use_news: bool = True) -> dict:
        ...
        if use_news:
            ...
            news_data = self.news_collector.get_comprehensive_news([symbol], days_back=7)
            news_sentiment = news_data['sentiment_summary'].get(symbol)
        
        # Train models on historical data
        print("Training prediction models...")
        training_results = self.predictor.train_models(stock_data_with_indicators, news_sentiment)
    ...
    def print_prediction_summary(self, result: dict):
        ...
        print(f"News Sentiment: {'Included' if result['news_sentiment_included'] else 'Not included'}")
        ...
        # Daily breakdown
        print(f"\nDAILY BREAKDOWN:")
        print(f"{'Day':<5} {'Direction':<10} {'Price':<10} {'Change':<10} {'Confidence':<12} {'Uncertainty':<12}")
        print(f"{'-'*60}")
        
        for day_pred in ensemble['daily_predictions']:
            print(f"{day_pred['day']:<5} "
        ...
        for model_name, perf in result['training_performance'].items():
            if 'error' not in perf:
                ...
                print(f"{model_name.replace('_', ' ').title()}: Failed - {perf['error']}")
